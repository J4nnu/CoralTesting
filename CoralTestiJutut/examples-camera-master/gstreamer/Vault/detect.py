...
led = GPIO("/dev/gpiochip2", 13, "out")  # pin 37 Led 220ohm vastuksen kautta pinniin 39/gnd
...
def generate_svg(src_size, inference_box, objs, labels, text_lines):
    ...
    for obj in objs:
        ...
        percent2= int(100*obj.score)
        label = '{}% {}'.format(percent, labels.get(obj.id, obj.id))
        svg.add_text(x, y - 5, label, 20)#Lisätään kohteeseen label obj.id perusteella 
        svg.add_rect(x, y, w, h, 'red', 2) #Asetetaa neliö tunnistetun kohteen ympärille.
        if obj.id==0 and percent2>=50:
          led.write(True)
          time.sleep(5)
          led.write(False)
          time.sleep(1)
...
def main():
    default_model_dir = '../all_models' #Kansio josta mallit ladataan
    default_model = 'mobilenet_ssd_v2_coco_quant_postprocess_edgetpu.tflite' #käytettävä malli
